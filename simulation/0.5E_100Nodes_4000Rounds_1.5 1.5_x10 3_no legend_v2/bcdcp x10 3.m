...
% ==== LEACH ELECTION OF CH ====
% for i=1:1:n
%    if(S(i).E>0)
%      temp_rand=rand;     
%      if ((S(i).G)<=0)
% 
%      %Election of Cluster Heads -- MST
...
% Select nodes that are above average energy 
E_sum = 0;
for i=1:1:n
    if S(i).E > 0
        E_sum = E_sum + S(i).E;
...
% for k=1:1:length(clusters)
%     for l=1:1:length(clusters{k})
%         text(clusters{k}(l).xd,clusters{k}(l).yd,string(k), 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'right');
%         
%         
%     end
% end
...
%         w = [w, sqrt((cluster_heads{i}.xd - cluster_heads{k}.xd)^2 + (cluster_heads{i}.yd - cluster_heads{k}.yd)^2)];
...
for j=1:1:length(cluster_heads)
    ...
    for k=1:1:length(clusters{j})
        ...
        if clusters{j}(k).E > 0 
            ...
            % Transfer from cluster to ch
            if clusters{j}(k).id ~= ch
                % Node energy
                node = clusters{j}(k).id;
                distance = DM(ch, node);%sqrt((S(ch).xd - S(node).xd)^2 + (S(ch).yd - S(node).yd)^2);

                % energy model based from leach
                if distance > do
                    S(node).E = S(node).E - ( ETX*2000 + Emp*2000*(distance)^4 );
                else
                    S(node).E = S(node).E - ( ETX*2000 + Efs*2000*(distance)^2 );
    ...
    for k=1:1:length(ch_path)-1
        ...
        % Deduct transmitting energy
        distance = DM(node, node_next);%sqrt((S(node).xd - S(node_next).xd)^2 + (S(node).yd - S(node_next).yd)^2);
        S(node).E = S(node).E - ( ETX*2000 + Emp*2000*(distance*distance) );

        % Deduct receiving energy
        S(node_next).E = S(node_next).E - (ERX + EDA) * 2000;
    end
    
    % main ch sends to BS
    % Node energy
    distance = DM(n+1, main_ch);%sqrt((S(n+1).xd - S(main_ch).xd)^2 + (S(n+1).yd - S(main_ch).yd)^2);
...
r=0:rmax;
% -------------------containers for plot----------------------------------
container_for_plot(v).first = STATISTICS2.PACKETS_TO_BS;
...
first = figure;
for i=1:length(container_for_plot)
    plot(container_for_plot(i).first*10,line_types(i,:),'LineWidth',1.5);
    hold on
end
...
second = figure;
for i=1:length(container_for_plot)
    plot(container_for_plot(i).second,container_for_plot(i).first *10,line_types(i,:),'LineWidth',1.5);
    hold on
end
legend(legend_f1,legend_f2,legend_f3,legend_f4,legend_f5,legend_f6,legend_f7,legend_f8,legend_f9,legend_f10,legend_f11,'Location','northwest');
title_label = 'Plot for Different Network Size values';
...
fourth = figure;
for i=1:length(container_for_plot)
    plot(container_for_plot(i).first *10,container_for_plot(i).third,line_types(i,:),'LineWidth',1.5);
    hold on
end
%legend(legend_f1,legend_f2,legend_f3,legend_f4,legend_f5,legend_f6,legend_f7,legend_f8,legend_f9,legend_f10,legend_f11,'Location','southwest');
title_label = 'Plot for Different Network Size values';
...
function [c1,ch1,c2,ch2] = twoClustering(c, DM)
    ...
    for i=1:1:length(c)-1
        for k=i+1:1:length(c)
            %dist_temp = sqrt((c(i).xd - c(k).xd)^2 + (c(i).yd - c(k).yd)^2);
            dist_temp = DM(i,k);
            if dist_temp > dist
                dist = dist_temp;
                ch1 = c(i);
    ...
    for i=1:1:length(c)
        d_ch1 = DM(c(i).id, ch1.id);%sqrt((c(i).xd - ch1.xd)^2 + (c(i).yd - ch1.yd)^2);
        d_ch2 = DM(c(i).id, ch2.id);%sqrt((c(i).xd - ch2.xd)^2 + (c(i).yd - ch2.yd)^2);
    ...
    if diff ~= 0
        ...
        if diff > 0
            ...
            dists = [];
            for i=1:1:length(c1)
                %dists = [dists, sqrt((c1(i).xd - ch2.xd)^2 + (c1(i).yd - ch2.yd)^2)];
        ...
        elseif diff < 0
            ...
            dists = [];
            for i=1:1:length(c2)
                %dists = [dists, sqrt((c2(i).xd - ch1.xd)^2 + (c2(i).yd - ch1.yd)^2)];
